...
@section('title', 'Operative CIS Payments - ' . $employee->full_name)
            ...
            <div class="col-lg-8">
                <div class="operative-welcome">
                    ...
                    <p class="operative-subtitle">
                        CIS Number: {{ $employee->cis_number ?? 'Not Assigned' }} • 
                        Status: <span class="status-badge verified">{{ $employee->cis_status ?? 'Unverified' }}</span>
                    </p>
                </div>
            </div>
            <div class="col-lg-4">
                <div class="operative-actions">
                    <div class="btn-group" role="group">
                        <a href="{{ route('cis.payments.create') }}?employee_id={{ $employee->id }}" class="btn btn-primary">
                        ...
                        <button type="button" class="btn btn-outline-primary" onclick="exportOperativeData()">
        ...
        <div class="card-body">
            <div class="row align-items-center">
                <div class="col-md-6">
                    ...
                    <div class="btn-group" role="group">
                        <a href="{{ route('cis.operative-payments', ['employee' => $employee->id, 'period' => 'ytd']) }}" 
                           class="btn btn-outline-primary {{ $period === 'ytd' ? 'active' : '' }}">
                            Year to Date
                        </a>
                        <a href="{{ route('cis.operative-payments', ['employee' => $employee->id, 'period' => 'quarterly']) }}" 
                           class="btn btn-outline-primary {{ $period === 'quarterly' ? 'active' : '' }}">
                            This Quarter
                        </a>
                        <a href="{{ route('cis.operative-payments', ['employee' => $employee->id, 'period' => 'monthly']) }}" 
...
function exportOperativeData() {
    ...
    link.setAttribute("download", "{{ str_replace(' ', '-', $employee->full_name) }}-cis-payments-" + period + "-" + year + ".csv");
...
function generateCisStatement(period, customStart = null, customEnd = null) {
    const employeeId = {{ $employee->id }};
    let url = `/cis/operative/${employeeId}/statement?period=${period}`;
                ...
                <div class="row mb-4">
                    <div class="col-md-4">
                        <div class="text-center">
                            ...
                            <small class="text-muted">CIS: {{ $employee->cis_number ?? 'Not Assigned' }}</small>
                        </div>
                    </div>
                    <div class="col-md-8">
